...
// Creating Availability (protected route - doctor only)
...
export const getMyAvailability = asyncHandler(async (req, res) => {
  ...
  if (!doctorAvailability || doctorAvailability.length === 0) {
    return res
      .status(200)
      .json(new ApiResponse(200, [], "No availability slot found"));
  }
...
export const updateAvailability = asyncHandler(async (req, res) => {
  ...
  if (maxPatients !== undefined)
    slot.maxPatients = maxPatients ?? slot.maxPatients;

  await slot.save();

  res
    .status(200)
    .json(new ApiResponse(200, slot, "Availability slot updated successfully"));
});
...
export const deleteAvailability = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, null, "SLot Deleted Successfully"));
});

// Toggle isActive status of slot (doctor only) -
export const toggleAvailabilityStatus = asyncHandler(async(req,res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, slot, "Availability status toggled successfully"));
})